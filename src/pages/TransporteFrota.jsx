...
import { 
  ...
  ChevronDown, ChevronUp // <-- Adicionados ícones de seta
} from 'lucide-react';
...
const TransporteFrota = () => {
  ...
  // <-- NOVO: Estado para controlar a expansão da coluna de férias
  const [feriasExpanded, setFeriasExpanded] = useState(false);
  ...
  const exportarPDF = () => {
    ...
    for (let i = 0; i < turnosOperacionais.length; i += 2) {
      ...
      doc.setFontSize(11);
      Para resolver isso, criei um **painel lateral deslizante (gaveta)** para a coluna de férias. Ela ficará recolhida no canto direito da tela e abrirá apenas quando clicada. 

Além dissodoc.setFont('helvetica', 'bold');
...
, **movi o botão de "Férias"** para baixo do nome do motorista e o deixei bem menor, o que libera muito espaço horizontal e evita que        columnStyles: { 0: { cellWidth: 10, halign: 'center' }, 2: { halign: 'center', fontStyle: o nome seja cortado.
...
const TransporteFrota = () => {
  ...
  const rankingPorTurno = useMemo(() => {
    ...
    motoristasBase.forEach(mot => {
      if (mot.<Medal size="{18}" className="md:w-5 md:h-5admin === true) return;

      const dadosViagem = rankingData.find(r ="> r.motorista_nome === mot.motorista);
      const total" /></div>;
    if (posicao === 2) return <div className="bg-amber-100 text-amber-700 p-1Viagens = dadosViagem ? dadosViagem.total_viagens : 0;
      const estaDeFerias = mot.status && mot.status..5 md:p-2 rounded-full shadow-sm flex-shrink-0"><Award size="{18}" className="md:w-5 md:h-toUpperCase() === 'FÉRIAS';
...
5"/></div>;
  ...
  const turnosOrdenadosTela = Object.keys(ranking        total: totalViagens,
        ferias: estaDeFerias
      };
      ...
      if (estaDeFerias) {
        ...
        const turno = mot.Turno || 'NÃO DEFINIDO';
        if (!gruposPorTurno).sort((a, b) => {
    if (a === 'FÉRIAS') return 1;
    if (b === 'FÉRI[turno]) grupos[turno] = [];
        grupos[turno].push(objMotorista);
      }
    });
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans relative overflow-x-hidden">
      ...
      <header className="bg-gradient-toisAdmin) return;

      const baseMot = motoristasBase.find(m => m.motorista === item.motorista_nome);
      const estaDeFer-r from-[#0f4c81] to-[#10b981] text-white p-4 md:p-6 shadow-lg flex justifyias = baseMot && baseMot.status && baseMot.status.toUpperCase() === 'FÉRIAS';
      const turnoGrupo = estaDeFerias ? 'F-between items-center sticky top-0 z-30">
        <div className="flex items-center gap-2 md:gap-4">
          <ÉRIAS' : (item.turno_nome || 'NÃO DEFINIDO'); if (!grupos[turnoGrupo]) grupos[turnoGrupo]="[];" button onClick="{()"> navigate('/')} className="hover:bg-white/20 p-2 rounded-full transition backdrop-blur-sm bg-white/10"><const jaExiste = grupos[turnoGrupo].some(g => g.nome === item.motorista_nome);
      
      if (!jaExiste) {
        ChevronLeft size={22} className="md:w-6 md:h-6" /></button>
          <div>
            <h1 className="font-blackgrupos[turnoGrupo].push({ 
          id: baseMot ? baseMot.id : null,
          nome: item.motorista_nome, 
 text-base md:text-2xl tracking-tight uppercase flex items-center gap-2 leading-tight"><Truck size="{20}" className="md:w-          total: item.total_viagens || 0,
          ferias: estaDeFerias 
        });
      }
    });

    if6 md:h-6"/> Ranking Frota</h1>
            <p className="hidden sm:block text-[10px] font-bold uppercase tracking-widest opacity-80 mt-1">Ranking Total da Operação</p>
          </div>
        </div>
        <button onClick={exportarPDF} className="bg- (grupos['FÉRIAS'].length === 0) delete grupos['FÉRIAS'];

    Object.keys(grupos).forEach(t => {
      white text-[#0f4c81] p-2.5 md:px-5 md:py-2.5 rounded-2xl font-black uppercase textif (t === 'FÉRIAS') {
        grupos[t].sort((a, b) => a.nome.localeCompare(b.nome));
-[10px] md:text-xs tracking-widest flex items-center gap-2 shadow-xl hover:scale-105 active:scale-9      } else {
        grupos[t].sort((a, b) => {
          if (filtroTipo === 'EXTRA') return a.total - b5 transition-all">
          <FileDown size="{18}"/> <span className="hidden sm:inline">Exportar PDF</span>
        </button>
      </header>

      <main className="p-3 md:p-6 max-w-7xl mx-auto space-y-4 md:space-y.total; 
          return b.total - a.total; 
        });
      }
    });
  ...
  const exportarPDF = () => {
    ...
    doc.setFillColor(...azulMarinho);
    doc.rect(0, 0, 210, 4          <div className="flex items-center gap-3 text-[#0f4c81] font-black uppercase tracking-widest text-[10px] md0, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold:text-sm px-2 w-full md:w-auto">
    ...
    div className="flex flex-col sm:flex-row gap-3 w-full md:flex-1 md:justify-end">
            ...
            <div className="flex items-center bg-slate-50 rounded-xl md:rounded-2xl border border-slate-200 p-1 px-3 flex-1 sm:flex-none">
              ...
              <select value={filtroTipo} onChange={e => setFiltroTipo(e.target.value)} className="bg-transparent font-bold text-[#0f4c81] text-xs md:text-sm outline-none py-2 uppercase cursor-pointer w-full">
                ...
                {opcoesFiltros.tipos.map(t => <option key={doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`RELATÓRIO: ${filtroTipo} | MÊS: ${filtroMes}`, 15, 28);
    doc.text(`EMITIDO EM: ${new Date().toLocaleDateString('pt-BR')}`, 15, 33);
    doc.setDrawColor(...verdeEsmeralda);
    ...
    for (let i = 0; i < turnosOperacionais.length; i += 2) {
      ...
      const minE = Math.min(...tEsquerda.map(m => m.total));
      const maxE = Math.max(...tEsquerda.map(m => m.total));
      const rangeE =t} value={t}>{t}</option>)}
              </select>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className maxE - minE;

      doc.setTextColor(...azulMarinho);
      doc.setFontSize(11);
      doc.setFont('helvetica', '="flex justify-center items-center h-64"><div className="animate-spin rounded-full h-12 w-12 border-b-4 border-[#10b981]"></div></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
            {turnosOrdenadosTela.map((turno) => {
              const motoristasRankebold');
      doc.text(`TURNO: ${nomeEsquerda}`, 15, yPos);
      ...
      autoTable(doc, {
        ...
        head: [['POS', 'MOTORISTA', 'TOTAL']],
        body: tEsquerda.map((m, idx) => [`${idx + 1}º`, m.nome, m.total]),
        stylesados = rankingPorTurno[turno];
              const isFeriasGrupo = turno === 'FÉRIAS';

              return (
                <div key={turno}: { fontSize: 7, cellPadding: 1.5 },
        headStyles: { fillColor: azulMarinho },
        columnStyles: { 0: className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-md border border-slate-100 overflow-hidden flex flex- { cellWidth: 10, halign: 'center' }, 2: { halign: 'center', fontStyle: 'bold' } },
        didcol h-fit">
                  
                  <div 
                    onClick={() => isFeriasGrupo && setFeriasExpanded(!feriasExpanded)}
                    className={`p-4 md:p-6 border-b flex justifyParseCell: (data) => {
...
-between items-center transition-colors ${
                      isFeriasGrupo 
                        ? 'bg-orange-50 border-orange-100 cursor-pointer hover:bg-orange-100' 
                        : 'bg-slate-50 border-slate-100'
                    }`}
                  >
                    <h2 className={`font-black text-sm md:text-lg uppercase tracking-widest flex items-center gap-2 ${isFeriasGrupo ? '              const txtColor = hslToRgb(hue, 75, 45);
              data.cell.styles.fillColor = bgColor;
              datatext-orange-600' : 'text-[#0f4c81]'}`}>
                      {isFeriasGrupo ? <Plane size="{18}" className.cell.styles.textColor="txtColor;" } else if (data.column.index="==" 0 && data.row.index="==" 0) {="text-orange-500"/> : <Clock size="{18}" className="text-[#10b981]"/>}
                      {turno}
      ...
      if (tDireita) {
        ...
        autoTable(doc, {
          ...
          columnStyles: { 0: { cellWidth: 10                    </h2>
                  ...
                  {(!isFeriasGrupo || feriasExpanded) && (
                    <div className="p-3 md:p-4 flex-1 overflow-y-auto max-h-[400px] md:max-h-[500px] space-y-2 md:space-y-3 custom-scrollbar">
                      {motoristasRankeados.map((mot, index) => {
                        ...
                        let estiloCard, halign: 'center' }, 2: { halign: 'center', fontStyle: 'bold' } },
          didParseCell: (data) => = {};
                        let classesCard = "flex items-center justify-between p-3 md:p-4 rounded-xl md:rounded-2xl transition-all {
            if (data.section === 'body') {
              const total = data.row.raw[2];
              if (filtroTipo === 'EXTRA') {
                const ratio = rangeD === 0 ? 0 : (total - minD) / rangeD;
                const hue = 120 - duration-300 ";
                        let corDoNumero = '';
      ...
      mot.total - minTrips) / range;
                          const hue = 120 - (ratio * 120);
                          
                          estiloCardyPos = Math.max(finalYEsquerda, finalYDireita) + 15;
      
      if (yPos > 240 && = { 
                            background: mot.total === 0 
                              ? `linear-gradient(to right, #f0fdf4, white)` 
 (i + 2 < turnosOperacionais.length || feriasDados.length > 0)) {
        doc.addPage();
        yPos = 20;
      }
    }
    ...
    if (feriasDados.length > 0) {
      doc.setTextColor(249, 11                              : `linear-gradient(to right, hsla(${hue}, 85%, 96%, 1), white)`, 
                            borderColor: mot.total === 0 ? '#bbf7d0' : `hsla(${hue}, 80%, 85%, 1)`, 
                            borderWidth: '1px', borderStyle: 'solid' 
                          };
                          corDoNumero = mot.total === 0 ? '#16a34a' : 5, 22);
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
  ...
  const renderizarPodio = (posicao) => {
    if (posicao === 0) return <`hsl(${hue}, 75%, 45%)`;
                        } else {
                        ...
                        return (
                          <div key={mot.nome} className={classesCard} style={estiloCard}>
                            <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                              ...
                              <div className="div className="bg-yellow-100 text-yellow-600 p-1.5 md:p-2 rounded-full shadow-sm flex-shrinktruncate min-w-0 flex-1 flex flex-col justify-center">
                                <p className={`font-black uppercase text-xs md:text--0"><Trophy size={18} className="md:w-5 md:h-5" /></div>;
    ...
    return <div className="bg-slate-50 text-slate-400 p-2 rounded-full font-black text-[10px] md:text-sm w-8sm truncate ${index === 0 && !isExtra && !isFeriasGrupo ? 'text-yellow-700' : 'text-[#0f4c81]'}`}>
                                  {mot.nome}
                                </p>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  {mot.id && (
                                    <button 
                                      ...
                                      className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest inline-flex items-center gap h-8 md:w-9 md:h-9 flex items-center justify-center border border-slate-200 flex-shrink-0">{posicao + 1}º</div>;
  };

  // Separa as Férias dos Turnos Principais para a interface principal
  const turnosPrincipais = Object.keys(rankingPorTurno).filter(t => t !== 'FÉRIAS').sort((a, b) => a.localeCompare(b));-1 transition-all shadow-sm w-max ${
                                        mot.ferias 
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] font-sans relative overflow-x-hidden">
      ...
      <header className="bg-gradient-to-r-center gap-3 md:gap-5 flex-shrink-0">
                              {!isFeriasGrupo && (
                                <div className="text-right">
 from-[#0f4c81] to-[#10b981] text-white p-4 md:p-6 shadow-lg flex justify-between                                  <p className="font-black text-xl md:text-2xl leading-none" style={{ color: corDoNumero }}>{mot.total}</p items-center sticky top-0 z-30">
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick>
                                  <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest mt={() => navigate('/')} className="hover:bg-white/20 p-2 rounded-full transition backdrop-blur-sm bg-white/10"><ChevronLeft-1">{isExtra ? 'Extras' : 'Viagens'}</p>
                                </div>
                              )}
                            </div>
                          </div>
                        );
                      })} size={22} className="md:w-6 md:h-6" /></button>
          <div>
            <h1 className="font-black text-
                    </div>
                  )}
                </div>
              );
      ...
      <style>{`
        ...
        .base md:text-2xl tracking-tight uppercase flex items-center gap-2 leading-tight"><Truck size={20} className="md:w-6 mdcustom-scrollbar::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 10px; }
        
        @media:h-6" /> Ranking Frota</h1>
            <p className="hidden sm:block text-[10px] font-bold uppercase tracking-widest opacity- (max-width: 640px) {
          main { padding-bottom: 5rem; }
        }
      `}</style>
    80 mt-1">Ranking Total da Operação</p>
          </div>
        </div>
        <button onClick={exportarPDF} className="bg-white text</div>
  );
};

export default TransporteFrota;
```-[#0f4c81] p-2.5 md:px-5 md:py-2.5 rounded-2xl font-black uppercase text-[10px] md:text-xs tracking-widest flex items-center gap-2 shadow-xl hover:scale-105 active:scale-95 transition-all">
          <FileDown size={18} /> <span className="hidden sm:inline">Exportar PDF</span>
        </button>
      </header>
